...
int main(){
	...
	vector<numberString> visualizedNumber;
	for(int i = 0; i < 10; i++){
		visualizedNumber.push_back(*(new numberString(i, height, width)));
	}
	string space = "";
	for(int i = 0; i < width; i++) 
		space += " ";
	for(int i = 0; i < 5 * height; i++){
		for (int j = 0; j < numberToConvert.length(); j++)
		{
			int printNumber = numberToConvert[j] - '0';
			visualizedNumber[printNumber].printLine(i);
			cout << space;	
		}
		cout << '\n';
	}
